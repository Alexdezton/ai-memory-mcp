 ...
 */
function getMemoryDirPath(projectRoot: string): string {
 ...
 */
function loadConfig(projectRoot: string): MemoryConfig {
 ...
 */
function checkWorkspaceLimits(projectRoot: string): void {
 ...
 */
function initProjectMemory(projectRoot: string): string {
  const memoryDir = getMemoryDirPath(projectRoot);
 ...
 */
function verifyWrite(projectRoot: string, filepath: string): string {
  const config = loadConfig(projectRoot);
  const memoryDir = getMemoryDirPath(projectRoot);
 ...
 */
function addLogEntry(projectRoot: string, filepath: string, content: string, mode: "prepend" | "append"): string {
  const resolvedPath = path.isAbsolute(filepath) ? filepath : path.resolve(projectRoot, filepath);
 ...
 */
function findTaskLineIndex(lines: string[], taskId: string): { index: number, indent: number } {
...
function manageTaskTree(
  projectRoot: string,
  action: "create_branch" | "add_subtask" | "close_task",
  taskId: string,
  parentId?: string,
  title?: string,
  status?: string
 ...
 */
function syncProjectState(projectRoot: string, component: string, description: string): string {
  const memoryDir = getMemoryDirPath(projectRoot);
 ...
 */
function logBrainstorm(projectRoot: string, topic: string, decisionsSummary: string): string {
  const memoryDir = getMemoryDirPath(projectRoot);
  const brainstormDir = path.join(memoryDir, "brainstorm");
 ...
 */
function cleanupWorkspace(projectRoot: string, filesToDelete: string[]): string {
  let log = "";
 ...
 */
function executeLoader(projectRoot: string, userTime: string): string {
  const memoryDir = getMemoryDirPath(projectRoot);
 ...
 * execute_eos()
 */
async function executeEos(projectRoot: string): Promise<string> {
  const memoryDir = getMemoryDirPath(projectRoot);
  const checkpointPath = path.join(memoryDir, "checkpoint-latest.json");
  ...
  try {
    ...
    switch (request.params.name) {
      ...
      case "add_log_entry": {
        ...
        const mode = request.params.arguments?.mode as "prepend" | "append";
      ...
      case "manage_task_tree": {
        const action = request.params.arguments?.action as "create_branch" | "add_subtask" | "close_task";
      ...
      case "cleanup_workspace": {
        const filesToDelete = request.params.arguments?.files_to_delete as string[];
