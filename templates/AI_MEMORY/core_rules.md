...
| Все остальные файлы в `AI_MEMORY` (логи, спецификации и др.) | Без лимита | — |
...
### 📂 Профессиональная структура файлов проекта

Для поддержания чистоты проекта **запрещено** размещать файлы исходного кода (`.css`, `.js`, `.ts`, `.jsx`, `.tsx`, `.scss`) непосредственно в корневом каталоге. Корень проекта предназначен только для файлов конфигурации (`package.json`, `.gitignore` и др.) и единой точки входа (например, `index.html`).

Все исходные файлы должны быть распределены по специализированным папкам:
* **Стили:** Должны находиться в `css/`, `styles/` или `src/styles/`
* **Логика и Скрипты:** Должны находиться в `js/`, `src/` или `scripts/`
* **Компоненты:** Должны находиться в `components/` или `src/components/`
* **Ассеты (изображения, шрифты):** Должны находиться в `assets/`, `images/` или `public/`
