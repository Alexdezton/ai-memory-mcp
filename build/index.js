    ...
    function scanDir(dir) {
        ...
        for (const file of files) {
            ...
            if (stats.isDirectory()) {
                ...
                if (filename === "mcp_ai_memory_config.json" ||
...
function initProjectMemory(projectRoot) {
    ...
    if (!fs.existsSync(specificationsDir))
        fs.mkdirSync(specificationsDir, { recursive: true });
    const moves = [
        {
 ...
 */
function logMcpAction(projectRoot, toolName, args, status, details) {
    try {
        const memoryDir = getMemoryDirPath(projectRoot);
 ...
 * manage_task_tree(action, task_id, parent_id, title, status)
 */
function manageTaskTree(projectRoot, action, taskId, parentId, title, status) {
    const memoryDir = getMemoryDirPath(projectRoot);
...
function executeLoader(projectRoot, userTime) {
    ...
    if (fs.existsSync(tasksPath)) {
        ...
        if (activeTaskMatch)
            activeTask = activeTaskMatch[1].trim();
        if (nextStepMatch)
    ...
    if (fs.existsSync(checkpointPath)) {
        try {
            ...
            if (activeTask === "None" || activeTask === "" || activeTask.includes("[ID]")) {
                if (checkpoint.active_task)
                    activeTask = checkpoint.active_task;
            }
